...
//-----------------------------------------------------------------------------
BumperKarts::BumperKarts() : LinearWorld()
{
...
//-----------------------------------------------------------------------------
void BumperKarts::getDefaultCollectibles(int *collectible_type, int *amount)
{
 ...
 */
bool BumperKarts::haveBonusBoxes()
{
 ...
 */
const std::string& BumperKarts::getIdent() const
{
    if(race_manager->getMinorMode() == RaceManager::MINOR_MODE_TIME_TRIAL)
        return IDENT_TTRIAL;
    else
